...
main = do
    devices <- enumerate
    case devices.decks of
        [] -> fail "no devices found"
        [ deck ] -> bracket (open deck) close doStuff3
...
doStuff2 deck = do
    ...
    buttons <- readButtonState deck 
    print "Found buttons"
    traceM $ show buttons
    return ()

doStuff' :: Deck a -> IO ()
doStuff' = go row1 row2 row3
    where
        row1 = row $ solidRGB 255 0 0
        row2 = row $ solidRGB 0 255 0
        row3 = row $ solidRGB 0 0 255
        go r1 r2 r3 deck = do
            deck' <- update deck $ SDPage' (r1, r2, r3)
            threadDelay 1_000_000
            go r2 r3 r1 deck'
    
-- Set Brightness
doStuff :: Deck a -> IO ()
doStuff deck = sendRaw deck $ setBrightness 100

-- Clear Image Brightness
doStuff3 :: Deck a -> IO ()
doStuff3 deck = sendRaw deck emptyJpg
